import {Control, customElements, ControlElement, LibPath, Types} from '@ijstech/base';
...
export class Icon extends Control {    
    ...
    protected init() {
        if (!this.initialized){
            ...
            this._updateIcon();
            const image: ImageElement = this.getAttribute('image', true);
            if (image && image.url) {
    ...
    static async create(options?: IconElement, parent?: Control){
