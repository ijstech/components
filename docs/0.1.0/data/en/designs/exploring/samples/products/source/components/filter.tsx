import { Control, Repeater, Module, customElements, ControlElement, Label, Panel, Styles, RadioGroup, ComboBox, Input } from '@ijstech/components';
...
type onChangedCallback = (type: string, value: any) => void;
...
export default class Filter extends Module {
  ...
  private renderLocation() {
    ...
    this.pnlLocation.appendChild(
      <i-radio-group
        ...
        onChanged={(target: RadioGroup) => this.onRadioChanged(target, 'location')}
      >
        {
          options.map(option => {
  ...
  private handleApply() {
    ...
    console.log(this.inputMin.value, this.inputMax.value)
    this.onSelectorChanged('min', this.inputMin.value);
    this.onSelectorChanged('max', this.inputMax.value);
  }
  ...
  private onSelectorChanged(type: string, value: any) {
    ...
    if (typeof this.onChanged === 'function') this.onChanged(type, value);
  }

  private onReset() {
    this.model.reset();
    this.renderUI();
    if (typeof this.onChanged === 'function') this.onChanged('reset', null);
  }
